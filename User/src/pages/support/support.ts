...
export class SupportPage {
  ...
  constructor(public navCtrl: NavController, public http:HTTP, public call: CallNumber, public settings: SettingsProvider, public viewCtrl: ViewController, public alert: AlertController, public eventProvider: EventProvider, public ph: ProfileProvider, public lp: LanguageProvider, public navParams: NavParams) {
     this.id = this.navParams.get('id')
  }

  ionViewDidEnter() {
    console.log('inregf')
    this.eventProvider.getSupportChatList(this.ph.id).on('value', snapshot => {
      this.eventList = [];
      console.log('sjiy')
    ...
    let alert = this.alert.create({
      ...
      buttons: [
        ...
        },
        {
          text: 'Send',
          handler: data => {
           this.send('Support Message from user with ID ' + this.ph.uid, data.Message, this.settings.support_email)
